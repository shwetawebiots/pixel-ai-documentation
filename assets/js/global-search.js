(function () {
   ...
   */
  function initSearch() {
    const searchInput = document.querySelector('.home-search input[type="search"]');
  ...
  function searchInIndex(query) {
    ...
    SEARCH_INDEX.forEach(page => {
      ...
      if (exactMatchCount > 0 || partialMatchCount > 0 || totalScore > 0) {
        ...
        const relevance = (exactMatchCount * 2 + partialMatchCount) / Math.max(queryWords.length, 1);
        const snippet = findRelevantSnippet(page.content, queryWords, normalizedQuery);
        results.push({
          page: page.title,
          url: page.url,
          snippet,
  ...
  function findRelevantSnippet(content, queryWords, normalizedQuery) {
    ...
    if (normalizedQuery && text.includes(normalizedQuery)) {
      ...
      const end = Math.min(text.length, phraseIndex + normalizedQuery.length + 120);
  ...
  function highlightText(text, query) {
    ...
    highlightedText = highlightedText.replace(/<mark>([^<]*)<\/mark>/gi, (match, content) => {
    ...
    placeholders.forEach((original, index) => {
      highlightedText = highlightedText.replace(placeholder + index + placeholder, original);
    });

    // Clean up any nested mark tags (shouldn't happen, but just in case)
    highlightedText = highlightedText.replace(/<mark>(.*?)<mark>(.*?)<\/mark>(.*?)<\/mark>/gi, "<mark>$1$2$3</mark>");
  ...
  function displayResults(results, query, container) {
    const html = results
      .map(result => {
        ...
        const urlWithQuery = result.url.includes("?") ? `${result.url}&q=${encodeURIComponent(query)}` : `${result.url}?q=${encodeURIComponent(query)}`;
